...
// Helper type for Turnstile verification response
...
export const processImage = action({
  args: {
    prompt: v.string(),
    style: v.string(),
    turnstileToken: v.string(), // Added Turnstile token
  },
  returns: v.object({
    storageId: v.id("_storage"),
  ...
  handler: async (ctx, args) => {
    ...
    if (!turnstileSecretKey) {
      ...
      throw new Error("Turnstile configuration error. Please contact support.");
    }
    ...
    try {
      const turnstileResponse = await fetch(verifyEndpoint, {
        ...
        body: `secret=${encodeURIComponent(
          turnstileSecretKey
        )}&response=${encodeURIComponent(args.turnstileToken)}`,
      });

      if (!turnstileResponse.ok) {
        // Handle non-2xx responses from Cloudflare
        const errorBody = await turnstileResponse.text();
        console.error(
          `Turnstile verification request failed with status ${turnstileResponse.status}: ${errorBody}`
        );
        throw new Error("Anti-bot verification check failed. Please try again later.");
      }

      verificationResult = await turnstileResponse.json();
    } catch (networkError: any) {
      console.error("Network error during Turnstile verification:", networkError);
      throw new Error(
        "Could not connect to anti-bot verification service. Please check your network and try again."
      );
    }

    if (!verificationResult.success) {
      console.error("Turnstile verification failed:", verificationResult["error-codes"]);
      // Log more details for admin, but give generic message to user
      throw new Error("Anti-bot verification failed. Please refresh and try again.");
    }

    // 2. If verification is successful, proceed with image generation
    const openaiApiKey = process.env.OPENAI_API_KEY;
    if (!openaiApiKey) {
      // Throw specific error if key is missing in environment
      console.error(
        `[processImage] ERROR: Environment variable 'OPENAI_API_KEY' not found or is empty in process.env! Throwing error.`
      ); // Log error before throw
      throw new Error(
        `OpenAI API Key (OPENAI_API_KEY) is not set in Convex environment variables.`
      );
    }
